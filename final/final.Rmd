...
\newpage
## Varyans Analizleri
Kategorik değişkenlerin analizi yapılabilmesi için varyans analizi (ANOVA) yöntemi kullanılmıştır. Bu yöntem ile farklı kategoriye sahip olan ülkelerin arasında oluşan farklılığın anlamlı mı yoksa anlamsız mı olduğu test edilmektedir. 
...
Çalışma daha yüksek gözlem hacmine ve veri kalitesine sahip, daha iyi dağılmış veri setleri ile teyit edilebilir ve geliştirilebilir. Zaman serisi analizleri gibi farklı analiz çeşitleri kullanılabilir, değişimlere olan duyarlılık ölçülebilir, farklı karşılaştırma grupları oluşturulup kategorize edilmiş çalışmalar yapılabilir, ulusal bazlı veriler yerine birey bazlı veriler (kesit veri) ile çalışmalar yapılabilir ve kullanılan basit lineer regresyon modeli değiştirilip tüm etkilerin aynı anda gözlemlendiği daha kapsamlı bir çalışma yapılabilir. Aynı zamanda farklı açıklayıcı değişkenlerin (kültürel faktörler, sosyal ilişkiler, çevresel koşullar gibi) modele katılmasıyla daha gerçekçi bir sonuç elde edilebilir. 
